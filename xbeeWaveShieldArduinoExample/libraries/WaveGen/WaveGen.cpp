  ...
  if (!playing && !generating) return;
    ...
    if (generating) {
        ...
        switch(indata) {
            ...
            case'4':
                out = t*((t>>5|s>>8)&63&(s>>4));
                t++;
                s++;
                break;
                
            case '5':
                out = t*((t>>5|t>>8)&63&(t>>4));
                t++;
                break;
        ...
        mcpDacSendBit(out, 15);
        mcpDacSendBit(out, 14);
        mcpDacSendBit(out, 13);
        mcpDacSendBit(out, 12);
        mcpDacSendBit(out, 11);
        mcpDacSendBit(out, 10);
        mcpDacSendBit(out,  9);
        mcpDacSendBit(out,  8);
        mcpDacSendBit(out,  7);
        mcpDacSendBit(out,  6);
 ...
 */
uint8_t WaveGen::create(FatReader &f) {
  // 18 byte buffer
 ...
 */
int16_t WaveGen::readWaveData(uint8_t *buff, uint16_t len) {
 ...
 */
void WaveGen::seek(uint32_t pos) {
 ...
 */
void WaveGen::setSampleRate(uint32_t samplerate) {
...
//------------------------------------------------------------------------------
/** Stop the player. */
void WaveGen::stop(void) {
  TIMSK1 &= ~_BV(OCIE1A);   // turn off interrupt
  playing->isplaying = 0;
  playing = 0;
}

void WaveGen::startGen(void) {
    generating = 1;
...
void WaveGen::stopGen(void) {
    generating = 0;
}
int WaveGen::isGen(void) {
    return generating;
}

void WaveGen::setInByte(char in) {
    indata = in;
}